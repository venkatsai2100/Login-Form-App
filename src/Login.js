...
function Login(props){
    ...
    const {Email,password}=data;
     ...
     const logIn=((e)=>{
         e.preventDefault();
        auth.signInWithEmailAndPassword(Email,password).then(user=>alert("Login success")).catch(err=>alert(err.message));
     });
return(
        ...
        <form className="form-group"> 
         <div className="d-flex  flex-column mt-5 p-4 justify-content-center shadow  align-items-start" style={{backgroundColor: "white",width:"25vw",height:"65vh",borderRadius:"8px"}}>
           <img src={logo} alt="Logo" style={{width:"60px",aspectRatio:1/1,margin:"10px auto",}}/>
            <h3 style={{color:"#A020F0"}}>Login</h3>
            ...
            <button onClick={logIn} className="w-100 btn mt-3 bg-primary p-2 text-white border-0">SIGN IN</button>
            <div className="d-flex mt-2 flex-row">
            <span style={{marginRight:"10px"}}>Don't have an account?</span>
            <a href="#" onClick={()=>{setLogin(!props.login)}}>Sign up</a>
            </div>
            </div>
        </form>
    </div>
